...
for reg = dirs
    ...
    F_opt = zeros(2, length(var_of_interest), length(windows), num_copies, runs); % one for Poiss and one for netw
    pc_opt = zeros(size(F_opt));
    mc_frs_1 = zeros(Nm, length(var_of_interest));
    mc_frs_2 = zeros(Nm, length(var_of_interest));

    covsum_evalues = zeros(2, Nm, length(var_of_interest), length(windows), num_copies, runs);
    covsum_evecs = NaN(2, Nm, Nm, length(var_of_interest), length(windows), num_copies, runs);

        corrcoefs = zeros(Nm, Nm, length(var_of_interest), length(windows), num_copies, runs);
        diff_mean_vector = zeros(2, Nm, length(var_of_interest), length(windows), num_copies, runs);
...
%                 F_opt(1, i_i, w_i) = (dot(w_poiss, diff_means_poiss))^2/(w_poiss'*cov_poiss*w_poiss)/wdow;
                F_opt(1, i_i, w_i, r_i) = (dot(w_poiss, diff_means_poiss))^2/(w_poiss'*cov_poiss*w_poiss);

                    [vecs, vals] = eig(cov_poiss);
                    [vals_s, ind] = sort(diag(vals));
                    vecs = vecs(:, ind);
                    covsum_evalues(1, 1:length(channel_keep_mc), i_i, w_i, r_i) = vals_s/wdow;
                    covsum_evecs(1, 1:length(channel_keep_mc), 1:length(channel_keep_mc), i_i, w_i, r_i) = vecs;
                
                    for mode_i = 1:length(channel_keep_mc)
                        mc_i = channel_keep_mc(mode_i);
                        Fopt_mode(1, mode_i, i_i, w_i, r_i) = ...
                            dot(vecs(:, mode_i), diff_means_poiss)^2/vals_s(mode_i);
                        Fopt_mc(1, mc_i, i_i, w_i, r_i) = ...
                            diff_means_poiss(mode_i)*w_poiss(mode_i);
                    end

                    diff_mean_vector(1, 1:length(channel_keep_mc), i_i, w_i, r_i) = diff_means_poiss;
                ...
                P1c = w'*stim1;
                P2c = w'*stim2;
                pc_opt(2, i_i, w_i, r_i) = ...
                    sum([P1c>c P2c<c])/num_samples/2;
%                 F_opt(2, i_i, w_i) = (dot(w, diff_means))^2/(w'*cov_sum*w)/wdow;
                F_opt(2, i_i, w_i, r_i) = (dot(w, diff_means))^2/(w'*cov_sum*w);

                    [vecs, vals] = eig(cov_sum);
                    [vals_s, ind] = sort(diag(vals));
                    vecs = vecs(:, ind);
                    covsum_evalues(2, 1:length(channel_keep_mc), i_i, w_i, r_i) = vals_s/wdow;
                    covsum_evecs(2, 1:length(channel_keep_mc), 1:length(channel_keep_mc), i_i, w_i, r_i) = vecs;
                
                    for mode_i = 1:length(channel_keep_mc)
                        mc_i = channel_keep_mc(mode_i);
                        Fopt_mode(2, mode_i, i_i, w_i, r_i) = ...
                            dot(vecs(:, mode_i), diff_means)^2/vals_s(mode_i);
                        Fopt_mc(2, mc_i, i_i, w_i, r_i) = ...
                            diff_means(mode_i)*w(mode_i);
                    end

                    diff_mean_vector(2, 1:length(channel_keep_mc), i_i, w_i, r_i) = diff_means;
                end
            end
        catch
        continue
